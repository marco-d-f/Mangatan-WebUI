...
const StructuredContent: React.FC<{ contentString: string }> = ({ contentString }) => {
    const parsedData = useMemo(() => {
        ...
        try {
            // Try to parse as JSON first (for structured JMdict data)
            return JSON.parse(contentString);
        } catch (e) {
            // FIX: If parsing fails, it is plain text. Return it as-is.
...
const ContentNode: React.FC<{ node: any }> = ({ node }) => {
    ...
    switch (tag) {
        ...
        // Default fallback for unknown structure
...
export const YomitanPopup = () => {
    ...
    useLayoutEffect(() => {
        ...
        let finalLeft: string | number = Math.min(x, viewportW - 360); // Prevent overflow right
        const MAX_HEIGHT = 450;

        if (y > viewportH * 0.6) finalTop = Math.max(10, y - MAX_HEIGHT - 10); // Flip up
    ...
    useEffect(() => {
        ...
        if (dictPopup.visible) document.addEventListener('mousedown', handleClickOutside);
                            ...
                            {entry.definitions.map((def, defIdx) => (
                                <div key={defIdx} style={{ display: 'flex', marginBottom: '12px' }}>
                                    ...
                                    <div style={{ flexGrow: 1 }}>
                                        {/* Tags & Dictionary Name Row */}
                                        <div style={{ marginBottom: '4px' }}>
                                            {/* Render "tags" like 'n' or 'forms' FIRST (Grey) */}
                                            {def.tags?.map((t, ti) => (
                                                <span key={ti} style={{ ...tagStyle, backgroundColor: '#666' }}>{t}</span>
                                            ))}
                                            
                                            {/* Render Dictionary Name (Purple) */}
